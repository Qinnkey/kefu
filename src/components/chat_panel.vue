		...
		<div class="chat_show">
				...
				<!--	<el-input prefix-icon="el-icon-search" size="small" v-model="tranSearch" @keyup.enter.native="search1" placeholder="搜索客服"></el-input>-->
						<ul>
							<li class="flex_between_v" v-for="(item,index) in tranList" @click="chooseTran(index)">
								...
								<span :class="item.custserverstatus != 'online' ? (item.custserverstatus == 'busy' ?  'state_light_2':'state_light_0'):'state_light_1'"></span>
								<div class="info">
									<p>{{item.custname}}</p>
									<p>当前接待人数：{{item.joinusernum}}</p>
								</div>
								<img class="li_icon" :src="item.custid != custId  ? (((chooseObj.choosed) &(chooseObj.custId==item.custid)) ? hasSelect:noSelect) : defSelect" width="20" style="width:20px"/>
							</li>
						</ul>
					</div>
					<span slot="footer" class="dialog-footer">
					    <el-button size="small" @click="TranVisible = false">取 消</el-button>
					    <el-button size="small" type="primary" :disabled="!chooseObj.custId  " @click="TranChangeSure">确 定</el-button>
					</span>
				</el-dialog>
			</div>
			<!--聊天消息面板-->
			<div class="chat_msg" ref="char_msg">
				<div v-for="item in msg_items" class="msg_item" :id="item.messageid">
					...
					<div v-if="item.sendway=='userway'" class="msg_lf flex_start">
						...
						<img class="headurl" :src="item.headurl" width="40" height="40"/>
						<div class="msg_txt" v-if ="item.msgtype=='text'" v-html="emoji(item.message)">
						</div>
				    	...
				    	<div v-if="item.msgtype == 'miniprogrampage'" class="product_infor">
    						<div class="card-desc">{{item.product_name}}</div>
    						<div class="product_detail">
    							<img :src="item.product_imgurl" alt="" />
    				...
    				<div class="message-item in persistnotice" v-if="item.sendway=='promptway'">
					    <div class="message-body">
					        <div class="inner">
					            <pre class="event">{{item.message}}</pre>
					        </div>
					    </div>
					</div>
			...
			<div class="input_msg">
				...
				<transition name="fade">
					<div class="emoji-box" v-show="showEmoji">
						...
						<div class="page_emotion box_swipe" id="page_emotion">
                            ...
                            <dt><ol id="nav_emotion" class="nav_emotion" style="display:none"></ol></dt><!-- 分页提示点 -->
                        </div>
					</div>
				</transition>
				<!--快速回复-->
				...
				<el-dialog title="新建快捷短语" :visible.sync="addReplyVisible" width="30%">
					<el-input type="textarea" v-model="quickReplyWords" :maxlength="500" :rows="7" resize="none" placeholder="请输入内容" @input="inputChange"></el-input>
					<div style="text-align: right; color: #ccc;">{{quickReplyNum}}/500</div>
					<span slot="footer" class="dialog-footer">
...
//					{user_icon: userIcon, sendway:'custway', username: "v字仇杀队-陶冶", custname:"dddddd", createtime: "2017-12-15", message:"我们的尊严不值什么钱，可它是唯一我们真正拥有的东西，是我们最后一寸领土，但在那一寸领土里，我们是自由的。"},
		...
		mounted(){
			...
			this.$root.Bus.$on("customer_list_choose", value=>{
				if (value){
					this.$nextTick(()=>{
						console.log(typeof this.$refs.char_msg);
			...
			this.$nextTick(()=>{
				// text = '您有未读【新消息】';
		  //       newtext();
				// document.getElementsByTagName('embed')[0].controls.play();
				form_emotion.rend(this);	
		...
		sockets:{
			message:function(res){
				if (res != '' || res != null || res.createtime != '' || res.createtime != null){
					...
					res.createtime = this.checkTime(a.getHours())+":"+this.checkTime(a.getMinutes())+":"+this.checkTime(a.getSeconds());
				}
		    ...
		    errPara:function(res){
		    	if (res && res != "102" && res !="100"){
		    		this.resultErr = res
		    		   this.$message({
				          showClose: true,
				          message: this.resultErr,
				          type: 'error'
				        });
		    	...
		    	if (res == "100"){
    				...
    				if (this.msgType != 'image'){
						...
						let time = this.checkTime(new Date().getHours())+":"+this.checkTime(new Date().getMinutes())+":"+this.checkTime(new Date().getSeconds());
...
//			},
			...
			chooseTran(index){
				...
				if (localStorage.custId !=this.tranList[index].custid ){
							//始终只能显示一个
				...
				console.log(this.chooseObj.custId,"sdfsdfsd")
			},
			//转接客服确定
			TranChangeSure(){
				//进行客服的转接,删除前台的对应的数据
			...
			//快速回复字数统计
			inputChange(val){
				this.quickReplyNum = 500 - val.length;
			},
			//确定添加快速回复
			addReplySure(){
			...
			submitMsg(){	
				if (!this.wordReplace(this.inputWords)=="") {
					//如果是小程序的话，那么就走另一个消息发送的socket接口
					if (localStorage.joinWay == 'small'){
						this.$socket.emit("routinemessage",{
...
<style type="text/css">
	.page_emotion{overflow:auto;max-height:200px;}
	.page_emotion dd{
	    display: inline-block;
	    float:left;
	    width:14.2%;
	...
	.list_emotion *[data-key]{
	    ...
	    height:24px;
	    margin: auto;
	    background:url(../../static/images/default.gif) no-repeat 0 0;
