  ...
  methods:{
    ...
    if (fileext == 'jpeg'||fileext =='gif'||fileext =='png'|| fileext =='jpg'){
      isIMG = true
        ...
        if (!isIMG) {
          Message.error('上传图片格式仅支持gif、jpeg、png、jpg 4种格式');
        }
            const isLt5M = file.size / 1024 / 1024  < 10;
        if (!isLt5M) {
          Message.error('上传图片大小不能超过10MB!');
        }
...
//      this.headurl='https://gss1.bdstatic.com/9vo3dSag_xI4khGkpoWK1HF6hhy/baike/w%3D268%3Bg%3D0/sign=e9449e382d9759ee4a5067cd8ac0242b/94cad1c8a786c9179e80a80cc23d70cf3bc75700.jpg';
    ...
    this.socket.on('connect',(res)=>{
        ...
        if (res =='connected'){
            console.log("user connect shopId=",this.shopId,';userid=',this.userId);
            this.socket.emit('userJoin', {shopId:this.shopId,userId:this.userId,joinWay:'weixin'});
        }
    })
  ...
  mounted(){
    let that = this;
    this.$nextTick(()=>{
        document.getElementsByTagName('input')[0].setAttribute('capture',"camera");
